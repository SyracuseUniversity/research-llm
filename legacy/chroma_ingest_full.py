...
EMBED_MODEL = getattr(config, "SENTENCE_TFORMER", None) or getattr(config, "EMBED_MODEL", "intfloat/e5-base-v2")
...
def build_paper_document(row: Tuple[Any, ...], works_fulltext_colname: str) -> Optional[Tuple[str, str, Dict[str, Any]]]:
    """
    Expected row fields from SQL below:
    paper_id,
    r_id,
    researcher_name,
    work_title,
...
def main() -> None:
    ...
    for r in rows:
        ...
        if pid is not None and str(pid).strip():
            unique_papers.add(str(pid).strip())
    print(f"Unique papers: {len(unique_papers)}")
    print(f"Works fulltext column: {works_fulltext_col}")
    ...
    for batch_idx in tqdm(
        ...
        for pid in ids_batch:
            ...
            chunks = _split_text(doc_text, max_chars=int(getattr(config, "CHUNK_MAX_CHARS", 12000)))
            if not chunks:
                continue
