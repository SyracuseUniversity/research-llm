...
DB_PATH = r"C:\codes\t5-db\researchers_fixed.db"  # adjust if needed
...
cur.execute("""SELECT info, doi, work_title FROM research_info""")

counts = Counter(); total = 0
for info, doi, title in cur.fetchall():
    combined = " ".join(filter(None, [str(info), str(doi), str(title)]))
    total += 1
    matched = False
    for label, pattern in PATTERNS.items():
        if pattern.search(combined):
            counts[label] += 1; matched = True; break
    if not matched:
        counts["unknown"] += 1
